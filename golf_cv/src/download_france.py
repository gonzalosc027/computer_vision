"""
Download Extra Training Data — IGN France (Géoportail)
───────────────────────────────────────────────────────
Descarga orthofotos aéreas de campos de golf franceses usando:
  • Overpass API (OpenStreetMap) — localiza campos en Francia
  • IGN France WMS (data.geopf.fr, completamente gratuito, sin token)
    Resolución: 20 cm/px  |  Licencia: Etalab (equivalente CC-BY)

Uso:
    python3 src/download_france.py
    python3 src/download_france.py --max-courses 400 --tiles-per-course 8
"""
...
# France bounding box (mainland only)
FR_BBOX = (41.3, -5.2, 51.1, 9.7)   # min_lat, min_lon, max_lat, max_lon

IMG_W, IMG_H  = 1600, 900
TILE_OVERLAP  = 0.25
PAUSE_S       = 0.7

OUT_DIR = RAW_DIR / "ign_france"
...
def fetch_golf_courses() -> list[dict]:
    min_lat, min_lon, max_lat, max_lon = FR_BBOX
    query = (
    ...
    for el in elements:
        ...
        seen.add(key)
        courses.append({
            "name": el.get("tags", {}).get("name", f"fr_course_{el['id']}"),
            "lat":  float(lat),
            "lon":  float(lon),
        })

    print(f"  Encontrados {len(courses)} campos en Francia")
    return courses
...
def verify_connection() -> bool:
    print("  Verificando conexión con IGN France …")
    img = download_tile(48.85, 2.33, 48.86, 2.345)
    if _is_valid_image(img):
        print("  Conexión OK ✓")
        return True
    print("  ERROR: no se pudo conectar con IGN France (data.geopf.fr).")
    return False


# ── main ─────────────────────────────────────────────────────────────────────

def download(max_courses: int = 400, tiles_per_course: int = 8):
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"\n{'='*55}")
    print(" Descarga de datos extra — IGN France")
    print(f"{'='*55}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Descarga orthofotos francesas de campos de golf — IGN France"
    )
    parser.add_argument("--max-courses",      type=int, default=400)
